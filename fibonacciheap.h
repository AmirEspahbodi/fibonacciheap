...
void FibonacciHeap::print_heap(/* string* spc */){
    ...
    /* !! Maybe it has a problem on memory leak! (IDK) */
    struct node_node
    {
        Node *node1, *node2;
    };
    ...
    while(true){
        ...
        *current = (*current)->right;
        ...
        while (*current==stack.back().node2) {
            /* so we pop this from stack */
            stack.pop_back();
            /* and update current pointer to Previous entry in stack*/
            if (!stack.empty()) current = &stack.back().node1;
            else return;
            /* Maybe the previous one is completely scrolled, so while loop is good */
        }
    }
    /*
    Node *iter=start;
...
void FibonacciHeap::Fib_Heap_Link(Node* y, Node** x, Node** start_node, Node** w){
    ...
    /*exchange x, y poitner if x.key is biger than y.key*/
...
Node* FibonacciHeap::Fib_Heap_Extract_Min(){
    if (this->min == NULL)
        return NULL;
    /* add H->min->childrens instead of H->min */
    if (this->min->child != NULL){
        Node *start = this->min->child;
        /* at first: all H.min.childrens.father -> null */
        do{
            start->father = NULL;
            start = start->right;
...
FibonacciHeap* FibonacciHeap::Fib_Heap_Union(FibonacciHeap* H1, FibonacciHeap* H2){
    ...
    if (H1->min!=NULL && H2->min!=NULL){
        ...
        /*heap 1 minimum right pointer should point heap 2 minimum right sibling*/
        H1->min->right = H2->min->right;
        /*heap 2 minimum right sibling's left pointer should point to heap 1 minimum*/
        H2->min->right->left = H1->min;
        /*heap 2 minimum right pointer should point to heap 1 minimum right sibling*/
        H2->min->right = h1mr;
        /* heap 1 minimum right sibling's left pointer should point to heap 2 minimum*/
        h1mr->left = H2->min;
    }
